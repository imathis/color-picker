 ...
 */
export const isInGamut = (oklchColor: {
  l: number;
  c: number;
  h: number;
  alpha?: number;
 ...
 */
export const getGamutMappedColor = (oklchColor: {
  l: number;
  c: number;
  h: number;
  alpha?: number;
...
export const rgbaToHex = ({ red, green, blue, alpha }: RGBColor): string => {
  ...
  const r = Math.max(
    0,
    Math.min(255, Math.round(parseFloat(String(red)) || 0)),
  );
  const g = Math.max(
    0,
    Math.min(255, Math.round(parseFloat(String(green)) || 0)),
  );
  const b = Math.max(
    0,
    Math.min(255, Math.round(parseFloat(String(blue)) || 0)),
  );
  ...
  if (alpha !== undefined && alpha < 1) {
    const a = Math.max(
      0,
      Math.min(255, Math.round((parseFloat(String(alpha)) || 0) * 255)),
    );
    colors.push(a);
  }

  return `#${colors.map((n) => n.toString(16).padStart(2, "0")).join("")}`;
};
...
export const createColorObject = (
  ...
  if ("hex" in currentParts) {
    ...
    "hue" in currentParts &&
    "saturation" in currentParts &&
    "luminosity" in currentParts
  ) {
    ...
    "hue" in currentParts &&
    "hsvSaturation" in currentParts &&
    "value" in currentParts
  ) {
    current.hue = currentParts.hue;
    ...
    "oklchLightness" in currentParts &&
    "oklchChroma" in currentParts
  ) {
  ...
  const isFullyBlack =
    hslvwb.luminosity < 0.1 ||
    hslvwb.value < 0.1 ||
    hslvwb.oklchLightness < 0.01;
  const isFullyWhite =
    (hslvwb.luminosity > 99.9 && hslvwb.saturation === 0) ||
    hslvwb.whiteness > 99.9 ||
    (hslvwb.oklchLightness > 0.99 && hslvwb.oklchChroma < 0.01);

  const isAchromatic =
    isLowSaturation || isLowChroma || isFullyBlack || isFullyWhite;
  ...
  if (
    model === "oklch" &&
    current.oklchLightness !== undefined &&
    current.oklchChroma !== undefined &&
    current.oklchHue !== undefined
  ) {
    const oklchColor: OKLCHColor = {
  ...
  const colorObj: ColorObject = {
    ...
    set: ({
      ...
      if (targetModel === "oklch" && colorObj.model === "oklch") {
        ...
        const isOklchOnlyAdjustment = adjustmentKeys.every((key) =>
          oklchKeys.includes(key),
        );
      ...
      // CONDITIONAL HUE PRESERVATION: Only preserve hue if result is achromatic
      // This prevents unwanted hue shifts when colors become gray/black/white
      // but allows natural hue updates during normal color space conversions
      const resultIsLowSaturation =
        newColorObj.saturation < 0.1 && newColorObj.hsvSaturation < 0.1;
      const resultIsLowChroma = newColorObj.oklchChroma < 0.02;
      const resultIsFullyBlack =
        newColorObj.luminosity < 0.1 ||
        newColorObj.value < 0.1 ||
        newColorObj.oklchLightness < 0.01;
      const resultIsFullyWhite =
        (newColorObj.luminosity > 99.9 && newColorObj.saturation === 0) ||
        newColorObj.whiteness > 99.9 ||
        (newColorObj.oklchLightness > 0.99 &&
          newColorObj.oklchChroma < 0.01);

      const resultIsAchromatic =
        resultIsLowSaturation ||
        resultIsLowChroma ||
        resultIsFullyBlack ||
        resultIsFullyWhite;
      ...
      if (resultIsAchromatic) {
        ...
        if (
          ...
          const hslColor: HSLColor = {
            hue: colorObj.hue,
            saturation: newColorObj.saturation,
            luminosity: newColorObj.luminosity,
            alpha: newColorObj.alpha,
          };
          const hsvColor: HSVColor = {
            hue: colorObj.hue,
            hsvSaturation: newColorObj.hsvSaturation,
            value: newColorObj.value,
            alpha: newColorObj.alpha,
          };
          const hwbColor: HWBColor = {
            hue: colorObj.hue,
            whiteness: newColorObj.whiteness,
            blackness: newColorObj.blackness,
            alpha: newColorObj.alpha,
          };
        ...
        if (targetModel === "oklch") {
          const oklchColor: OKLCHColor = {
            oklchLightness: newColorObj.oklchLightness,
            oklchChroma: newColorObj.oklchChroma,
            oklchHue: colorObj.oklchHue,
            alpha: newColorObj.alpha,
          };
